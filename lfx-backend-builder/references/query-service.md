...
| `filters_all` | []string | AND filter — all values must match. Explicit preferred alias for `filters` |
| `filters_or` | []string | OR filter — at least one value must match (format: `field:value`, against `data.*`) |
